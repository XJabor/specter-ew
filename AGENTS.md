...
- EP (Electronic Protection) mode: green EP nodes (`epNodes[]`, IDs prefixed `EP`) each hold one or more named sub-systems; clicking Calculate fires sequential `/calculate_es_terrain` calls (one per system) and renders color-coded terrain rings; sidebar shows EP-specific parameters (terrain, enemy RX sensitivity) while hiding EA controls. EP node system cards support both manual `+ Add System` and Library-based `Add From Library`, copying template name/frequency/power/gain/antenna type/beamwidth/height into editable EP system rows. Each system row exposes an Omni/Directional antenna select; directional reveals azimuth/beamwidth inputs (`.sys-dir`) and shapes that system's ring per bearing. The card/row CSS (`.sys-card`, `.sys-row`, `.sys-params`, `.sys-dir`, `.sys-label`) is shared with the red ENEMY SYSTEMS list; `.ep-theme` / `.red-theme` supply the only colour differences.
- Red nodes can carry extra frequency systems alongside their own equipment ring: a per-node ENEMY SYSTEMS card list in the EA workbench, each system rendering its own coloured terrain ring on a manual Calculate. See the design note below.
...
- **rx_sensitivity defaults are intentionally split**: the frontend always sends `rx_sensitivity` explicitly, so the backend's `DEFAULT_RX_SENSITIVITY_DBM = -90` (app.py) is reachable only by direct API callers and must not be changed casually — it is part of the public API contract (pinned by a regression test). Frontend defaults differ by role: −100 dBm for the generic friendly sensor (`DEFAULT_GENERIC_FRIENDLY_SENSOR_SENSITIVITY_DBM`), −90 dBm for the jammer-footprint reference threshold and the EP enemy-RX form field. These are three different physical assumptions, not an inconsistency.
...
- **Red node extra systems**: a red node's own `equipment` still drives one ES ring, comms links, J/S pairs, and blue sensor coverage. On top of that it carries `node.systems[]` — named, ring-only emitters that mirror the EP system shape field-for-field (`freqMhz`/`txPowerW`/`txGainDbi`/antenna trio, plus their own `layer`/`label`/`polygonPoints`/`rangeKm`). They deliberately participate in *nothing* except their own ring: no links, no jamming, no sidebar equipment editor, no `compatibleRedTransmitters`. Managed in `enemy_systems.js` by `addSystemToRedNode()`, `addLibrarySystemToRedNode()`, `removeSystemFromRedNode()`, `redUpdateSysParam()` / `redUpdateSysAntennaType()` (clamps via `RED_SYS_PARAM_NORMALIZERS`, matching the EA setters, and applies the same 360 → 90 beamwidth snap as EP when a system is switched to `directional`), `calculateRedNodeSystems()`, and `updateRedSystemsWorkbench()`. Three deliberate divergences from EP: the reference threshold comes from `selectedSensorReference()` (the same sensor the primary ES ring uses) rather than a dedicated sidebar field; the circle-fallback branch fills `polygonPoints` via `circleToPolygon()` so overlap and KML always work; and both setters also call `renderOverlapControls()`, because red system rings feed overlap analysis while EP rings do not (`activeSensorCoverages()` returns blue, red, and red-system coverages only). Per-node system ids/colours come from the monotonic `nextSystemIndex()` in `map_core.js` (shared with EP systems), so deleting a system never lets the next one collide with a surviving sibling. Rings surface in `activeSensorCoverages()` (synthetic sensor id `red-sys-<sysId>`, tagged `coverage.redSystem` so KML routes them to their own coloured folder instead of the flat-red ES folder), and are invalidated on any parameter edit, drag, or MGRS jump.
- **EP mode**: toggled by `toggleEpMode()` which sets `epModeActive` and adds/removes the `ep-mode` CSS class on `<body>`. `.ea-only` elements are hidden and `.ep-only` elements shown via CSS rules when `body.ep-mode` is active. EP nodes (`epNodes[]`, IDs prefixed `EP`) are placed by `placeEpNode()` and managed by `addSystemToEpNode()`, `removeSystemFromEpNode()`, `calculateEpNode()`, `clearEpNodeRings()`, and `removeEpNode()`. Systems use colors from `EP_COLORS[]` (8-color palette, assigned by index mod 8). Sequential `for...of` loop in `calculateEpNode()` avoids exceeding the OpenTopoData 1 req/sec rate limit. EP nodes are included in `updateMGRSTooltips()`, Clear All, and Center on Nodes (including ring bounds). Per-system edits go through `epUpdateSysParam()` (normalizes `antennaAzimuth` mod 360 and clamps `antennaBeamwidth` to `[1,360]` via `EP_PARAM_NORMALIZERS`, matching the EA setters in `nodes_links.js`) and `epUpdateSysAntennaType()`; both clear the node's rings and re-render, since any parameter change invalidates the computed polygon. Switching a system to `directional` while its beamwidth is still 360 snaps it to 90 — `directional_gain_db()` only sheds ~3 dB at 180° off boresight at 360° HPBW, so an unsnapped "directional" system would render as a round ring.
- **Collapsible workbench sections**: `bindSectionMinimize(btnId, bodyId)` in `app_init.js` wires all three Ops-tab carets (`btn-minimize-links` / `link-statuses-body`, `btn-minimize-red-systems` / `red-systems-list`, `btn-minimize-overlap` / `overlap-body`). The button owns the caret glyph, the body div owns `style.display` — so the collapsed state survives the `innerHTML` rebuilds that `renderResults()`, `updateRedSystemsWorkbench()`, and `renderOverlapControls()` perform, since none of them touch the wrapper's display. A collapsible section therefore needs a single body element wrapping everything under its label: `#overlap-body` exists purely for this, and buttons inside it that manage their own inline `display` (`btn-toggle-corners`, `btn-clear-overlap`) are unaffected because the parent hides the whole group without rewriting their style.
