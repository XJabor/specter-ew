...
**Frontend** (`static/js/`, vanilla JS split into eight classic scripts — no modules, no build step). Script order in `templates/index.html` is load-bearing; top-level `let`/`const` share the global lexical scope across files, so a file loaded out of order throws load-time TDZ ReferenceErrors:
...
6. `ep_mode.js` — EP nodes/systems/workbench (EP state lives in map_core).
7. `scenario_io.js` — scenario persistence, autosave, download, KML export.
8. `app_init.js` — **loaded last; the only file (besides map_core's map creation) that runs top-level code**: every `addEventListener`, Leaflet control registration, and bootstrap call lives here so all functions/state exist before wiring runs. Files 3–7 must contain declarations only.
